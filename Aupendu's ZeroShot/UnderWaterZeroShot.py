...
def test(args):
	...
	for i in range(len(InputImages)):
		...
		Ig = torch.mean(Input[0, 1, :, :])
		Ir = torch.mean(Input[0, 0, :, :])

		Input[0, 0, :, :] = Input[0, 0, :, :] + (Ig - Ir)*(1 - Input[0, 0, :, :])*Input[0, 1, :, :]
		...
		for k in tqdm(range(itr_no), desc="Loading..."):
			...
			loss =   1000*lossCLR + lossT + lossA + 0.001*lossMx + 0.001*lossMn + 0.001*lossI 
